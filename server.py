    ...
    try:
        ...
        if places:
            ...
            if (placesRequired <= int(competition['numberOfPlaces'])
                and placesRequired > 0) and (int(club['points']) > 0
                                             and (int(club['points'])
                                                  >= placesRequired and
                                                  placesRequired < 13)):
                ...
                flash('Felicitation! vous venez de reserver {} places'
                      .format(placesRequired))
            else:
                flash('echec lors de la reservation de {} places !'
                      .format(placesRequired))
        else:
            flash('Aucune place n\'a été sélectionnée!')
        return render_template('welcome.html', club=club,
                               competitions=competitions_with_valid_date,
                               competition_done=competitions_done)
